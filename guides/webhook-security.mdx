...
| --- | --- | --- | --- |
| REST API | `x-signature`, `x-timestamp` | Epoch **ms** | `timestamp + METHOD + path + body` |
...
For authenticated developer API calls (`/api/v1/*`), HMAC uses your **API secret** with millisecond timestamps and method+path signing. See [API keys and HMAC](/guides/api-keys-hmac).
