...
Echo Zero uses **three distinct signing schemes** depending on direction and transport. Using the wrong algorithm is the most common integration mistake.
...
When your agent has a `webhookUrl`, Echo Zero POSTs execution results after trade attempts:
...
When Echo Zero hosts signal ingestion for Telegram or Discord bots, provider-specific secrets apply. These routes do **not** use developer API keys.
