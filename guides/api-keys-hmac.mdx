...
SIG=$(printf '%s' "${TS}GET${PATH}" | openssl dgst -sha256 -hmac "$SECRET" | awk '{print $2}')

curl -sS "https://mcp.echozero.app${PATH}" \
  -H "x-api-key: $API_KEY" \
  -H "x-timestamp: $TS" \
  -H "x-signature: $SIG"
```
...
SIG=$(printf '%s' "${TS}POST${PATH}${BODY}" | openssl dgst -sha256 -hmac "$SECRET" | awk '{print $2}')

curl -sS -X POST "https://mcp.echozero.app${PATH}" \
  -H "x-api-key: $API_KEY" \
