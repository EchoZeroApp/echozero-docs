...
## 3. Create a developer agent (optional)

```bash
curl -sS -X POST https://mcp.echozero.app/api/v1/developers/agents \
  -H "x-api-key: $EZ_API_KEY" \
...
For webhook agents, save `inboundWebhookSigningSecretReveal` from the response — it is shown only once. Use it to sign inbound signals per the [Webhook security guide](/guides/webhook-security).
